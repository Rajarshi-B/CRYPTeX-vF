...
R1_AES = tk.Radiobutton(root, text="AES-256|PBKDF2(SHA-512)++", variable=var, value=1,
                        command=MODE_Config)
R1_AES.grid(row=0, column=1, padx=10, pady=5, sticky=tk.W)


R2_VigAES = tk.Radiobutton(root, text="VigenWrap(AES-256|PBKDF2(SHA-512)++)", variable=var, value=2,
                           command=MODE_Config)
R2_VigAES.grid(row=1, column=1, padx=10, pady=5, sticky=tk.W)

R3_File = tk.Radiobutton(root, text="EnDyc(Binary File)-Vigen(AES-256)", variable=var, value=3,
                         command=MODE_Config)
R3_File.grid(row=2, column=1, padx=10, pady=5, sticky=tk.W)
